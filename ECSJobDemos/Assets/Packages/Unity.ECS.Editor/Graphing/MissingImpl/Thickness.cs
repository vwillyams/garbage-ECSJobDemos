...
namespace System.Windows
{
    ...
    public struct Thickness : IEquatable<Thickness>
    {
        ...
        internal bool IsValid(bool allowNegative, bool allowNaN, bool allowPositiveInfinity, bool allowNegativeInfinity)
        {
            ...
            if (!allowNaN)
            {
                if (DoubleUtil.IsNaN(Left) || DoubleUtil.IsNaN(Right) || DoubleUtil.IsNaN(Top) ||
                    DoubleUtil.IsNaN(Bottom))
                    return false;
            }
            ...
            if (!allowPositiveInfinity)
            {
                if (Double.IsPositiveInfinity(Left) || Double.IsPositiveInfinity(Right) ||
                    Double.IsPositiveInfinity(Top) || Double.IsPositiveInfinity(Bottom))
                {
                    return false;
                }
            }
            ...
            if (!allowNegativeInfinity)
            {
                if (Double.IsNegativeInfinity(Left) || Double.IsNegativeInfinity(Right) ||
                    Double.IsNegativeInfinity(Top) || Double.IsNegativeInfinity(Bottom))
                {
                    return false;
                }
            }
