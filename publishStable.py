...
def is_package_changed(package_folder, package_name):
    ...
    installed_path = os.path.join("node_modules", package_name)
    print "Comparing files between {0} and {1}".format(installed_path, package_folder)
    match, mismatch, errors = filecmp.cmpfiles(installed_path, package_folder, [])
...
def get_next_package_version(package_name, major, minor, patch):
    # type: (str) -> str
    ...
    new_major = int(highest_version_trimmed[0])
    new_minor = int(highest_version_trimmed[1])
    new_patch = int(highest_version_trimmed[2])
    if major:
        new_major += 1
    if minor:
        new_minor += 1
    ...
    print "New version is changed from {0} to {1}".format(highest_version, new_version)
...
def publish_new_package(package_path, version):
    ...
    try:
        ...
        npm_cmd("--no-git-tag-version version {0}".format(version), None, False).strip()
...
def main():
    ...
    try:
        ...
        git_cmd("remote add stable {0}".format(dest_repo), True)
        print "Removing $srcRepo from remote of repo"
        git_cmd("remote remove origin", True)
        ...
        for package_path in packages:
            ...
            package_version = get_next_package_version(package_name, False, False, True)
            ...
            if changed is True:
                ...
                package_version = publish_new_package(package_path)
            else:
                publish_new_package(package_path, package_version)
                print "No change detected in the repo. The version {0} will be used in the project".format(
                    package_version)
            ...
            modify_manifest(package_path, package_version)
        ...
        if release_number != 1:
            ...
            pass
        print "Pushing squashed branch {0} to remote stable".format(dest_branch)
        git_cmd("push stable {0}".format(dest_branch), True)
        os.chdir(root_dir)
    except subprocess.CalledProcessError as e:
        print e
        print e.output
